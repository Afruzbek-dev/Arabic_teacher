...
function Shell() {
  ...
  // Auto-set user name from auth profile if onboarding
  useEffect(() => {
    if (profile && !state.user.onboarded && profile.displayName) {
      // Pre-fill the name for the landing page onboarding
    }
  }, [profile])

  const navigate = useMemo(
    () => (nextView, nextParams = {}) => {
      // Guard: don't navigate to lesson without a lessonId
      if (nextView === 'lesson' && !nextParams.lessonId) {
        console.warn('Navigate to lesson with no lessonId — redirecting to map')
        nextView = 'map'
        nextParams = {}
      }
  ...
  switch (view) {
    ...
    case 'map':
      ...
      break
    case 'lesson':
      screen = <LessonPlayer key={params.lessonId} lessonId={params.lessonId} />
      break
    case 'results':
